...
contract('ChainList', function(accounts) {
  ...
  it("should be initialized with empty values", function(){
    return ChainList.deployed().then(function(instance){
      ...
      // Chai assertians to confirm state variables are properly initialized
  ...
  it("should let us sell a first article", function(){
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._name, articleName1, "article must be " + articleName1);
      assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei(articlePrice1, "ether"), "event article must be " + web3.toWei(articlePrice1, "ether"));
  ...
  it("should let us sell a second article", function(){
    return ChainList.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._seller, seller, "event seller must be " + seller);
      assert.equal(receipt.logs[0].args._name, articleName2, "article must be " + articleName2);
      assert.equal(receipt.logs[0].args._price.toNumber(), web3.toWei(articlePrice2, "ether"), "event article must be " + web3.toWei(articlePrice2, "ether"));
...
// buy the first article
  it("should buy an article", function() {
    return ChainList.deployed().then().then(function(instance) {
      chainListInstance = instance;
      ...
      // check the effect of buy on the balances of the buyer and seller, accounting for gas
      ...
      assert.equal(data.toNumber(), 2, "there should still 2 articles in total")
    });
  });
});
